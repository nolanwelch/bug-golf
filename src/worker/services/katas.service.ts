 ...
 */
export async function getKataByDate(date: Date): Promise<Kata | null> {
  const prisma = getPrisma();
 ...
 */
export async function getKataById(id: number): Promise<Kata | null> {
  const prisma = getPrisma();
  return prisma.kata.findUnique({ where: { id } });
}

export async function addKata(data: Omit<Kata, "id">): Promise<Kata> {
  const prisma = getPrisma();
